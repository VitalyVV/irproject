import urllib.request, urllib.parse, urllib.error, re, html.parser
import pickle
...
for i in range(5):
    plt.figure(i)
    plt.plot(list(range(len(Original_Scores[i]))),sorted(Original_Scores[i]), 'r',label='original')
    plt.plot(list(range(len(Revised_Scores[i]))),sorted(Revised_Scores[i]), 'g', label='revised')
    plt.plot(list(range(len(Corrected_Scores[i]))),sorted(Corrected_Scores[i]), 'b', label='corrected')
    plt.legend(loc=0)
